...
const getInnertubeUrl = pathToFileURL(resolve(distDir, 'internal', 'getInnertube.js')).href;
const createSabrUrl = pathToFileURL(resolve(distDir, 'internal', 'createSabr.js')).href;
const extractorUrl = pathToFileURL(resolve(distDir, 'index.js')).href;
const { getInnertube } = await import(getInnertubeUrl);
const { createSabrStream } = await import(createSabrUrl);
...
async function main() {
  ...
  const hasCookie = !!process.env.YOUTUBE_COOKIE;
  ...
  console.log(`[smoke] Registered extractor: ${registeredId}. Total extractors: ${player.extractors.size}`);
  ...
  if (!videoId) {
    ...
    const firstVideo = results.results.find((v) => v.type === 'Video' && v.video_id);
    if (!firstVideo) throw new Error('No video result found in search');
    videoId = firstVideo.video_id;
    console.log(`[smoke] Selected first result: ${firstVideo.title?.text || firstVideo.title}`);
  } else {
    console.log(`[smoke] Using provided video ID: ${videoId}`);
  }

  // Test 1: URL extraction via player.search
  const urlToTest = 'https://www.youtube.com/watch?v=COz9lDCFHjw';
  // Use the extractor directly for deterministic behavior
  const resUrlInfo = await registered.handle(urlToTest, { requestedBy: 'smoke' });
  ...
  // Test 2: text search via player.search
  const resSearchInfo = await registered.handle(arg || 'lofi hip hop', { requestedBy: 'smoke' });
  if (!resSearchInfo || !resSearchInfo.tracks || resSearchInfo.tracks.length === 0) {
    throw new Error('Text search returned no tracks');
  }
  console.log(`[smoke] Text search OK. First track: ${resSearchInfo.tracks[0].title}`);

  if (!shouldStream) {
    console.log('[smoke] Streaming test disabled (set SMOKE_STREAM=1 to enable).');
    return;
  }
  if (!hasCookie) {
    console.log('[smoke] SMOKE_STREAM=1 set but no YOUTUBE_COOKIE provided. Streaming skipped.');
    return;
  }
